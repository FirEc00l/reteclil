        ...
        if request.method == 'POST':
            ...
            if title is None:
                ...
                if f and allowed_file(f.filename):
                    ...
                    query = "INSERT INTO file VALUES(NULL, '%s','%s','%s', '%s')" % (
                        f.filename, id_user, id_sub, description)
                    db.query_db(query)
                ...
                datetime = str(day) + "-" + str(month) + "-" + \
                    str(year) + " " + str(time)

                query = "INSERT INTO event VALUES(NULL, '%s', '%s', '%s', '%s', '%s', '%s')" % (
                    title, datetime, description, place, address, id_user)
                db.query_db(query)
